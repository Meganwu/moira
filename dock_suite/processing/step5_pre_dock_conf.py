...
if __name__ == '__main__':
    outfile_good=open('step5_good.txt','w')
    outfile_bad=open('step5_bad.txt','w')
    pdb_list=read_pdbid()
    ...
    for j,i in enumerate(pdb_list,start=1):
        dock_name=i+'dock'
        os.chdir(dock_name)
        total=cent[cent[1]==i].values
        x=total[0][2]
        y=total[0][3]
        z=total[0][4]
        filename=i+'.conf'
        pro_name=i+'_chain.pdbqt'
             
        try:
           write_dock_conf(filename,pro=pro_name, c_x=x, c_y=y, c_z=z, s_x=28, s_y=28, s_z=28,num_mode=10,num_exh=16)
