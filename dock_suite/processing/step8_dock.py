...
if __name__ == '__main__':
    outfile_good=open('step8_good.txt','w')
    outfile_bad=open('step8_bad.txt','w')
    pdb_list=read_pdbid()
    ...
    for j,i in enumerate(pdb_list,start=1):
        ...
        try:
           for cid in range(confs):
               ...
               dock_file=i+str(cid)
               os.chdir(dock_file)
               shutil.copy('../../../dock_file/prepare_receptor4.py','./')
               shutil.copy('../../../dock_file/prepare_ligand4.py','./')
               batch_vina(pro_file,dock_cid,filename)
               os.chdir('../')
